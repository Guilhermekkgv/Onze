...
function Linux.SaveCustomizationSettings()
    local success, result = pcall(function()
        local customizationData = {
            ToggleIconAssetId = Linux.ToggleIconAssetId
        }
        local json = HttpService:JSONEncode(customizationData)
        if writefile then
            if not isfolder(Linux.ConfigFolder) then
                makefolder(Linux.ConfigFolder)
            end
            writefile(Linux.ConfigFolder .. "/customization.json", json)
            return true
        else
            return false, "writefile function not available"
        end
    ...
    if not success then
        print("Error saving customization settings:", result)
    end
    return success, result
end

function Linux.LoadCustomizationSettings()
    local success, result = pcall(function()
        if readfile and isfile(Linux.ConfigFolder .. "/customization.json") then
            local json = readfile(Linux.ConfigFolder .. "/customization.json")
            local customizationData = HttpService:JSONDecode(json)
            Linux.ToggleIconAssetId = customizationData.ToggleIconAssetId or "rbxassetid://119686822323882"
            return true
        else
            return false, "Customization settings file not found"
        end
    end)
    if not success then
        print("Error loading customization settings:", result)
    end
    return success, result
end
    ...
    local success, result = pcall(function()
        if listfiles and isfolder(Linux.ConfigFolder) then
            ...
            for _, file in pairs(files) do
                ...
                if configName and configName ~= "autoload" and configName ~= "privacy" and configName ~= "accent" and configName ~= "customization" then
...
function Linux.Create(config)
    ...
    Linux.LoadCustomizationSettings()
    ...
    local function UpdateToggleIcon(newAssetId)
        if ToggleButton then
            ToggleButton.Image = newAssetId
        end
    end
                ...
                if confirmationConfig.Buttons then
                    for i, buttonConfig in ipairs(confirmationConfig.Buttons) do
                        local DialogButton = Linux.Instance("TextButton", {
                            Parent = ButtonsFrame,
                            BackgroundColor3 = i == 1 and Linux.Accents[Linux.CurrentAccent] or Color3.fromRGB(60, 60, 70),
                            BorderSizePixel = 0,
    ...
    if config.ConfigSave ~= false then
        ...
        SettingsTab.Section({Name = "Customization"})
        local toggleIconInput = SettingsTab.Input({
            Name = "Icon Toggle UI",
            Description = "change the toggle UI icon",
            Placeholder = "rbxassetid://",
            Default = Linux.ToggleIconAssetId,
            Finished = true,
            Callback = function(text)
                if text and text ~= "" then
                    Linux.ToggleIconAssetId = text
                    Linux.SaveCustomizationSettings()
